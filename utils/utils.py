    ...
    def __print(self, dictionary, tab="  ", level=0, list_lim=10, quote=""):
        ...
        for key, item in dictionary.items():
            ...
            # If item is a list, if the list contains a list/tuple/Namespace or is long, write in separate lines
                ...
                if any([any([isinstance(i, t) for t in (list, tuple, Namespace)]) for i in item]) or len(
                        item) > list_lim:
                    lines.append("%s%s:" % ((level * tab), key))
                    for i in item:
...
class Dataloader(object):
    ...
    def __getitem__(self, index: int):
        ...
        image = Namespace(
            ...
            image=cv2.imread("/".join((self.img_root, "images", self.metadata.iloc[index].filename)),
        ...
        label = Namespace(
            ...
            type_id=self.key["Type"].index(self.metadata.iloc[index].ship_type),
            class_id=self.key["Class"].index(self.metadata.iloc[index].ship_class),
            image=cv2.imread("/".join((self.img_root, "labels", self.metadata.iloc[index].labelname)),
                             cv2.IMREAD_UNCHANGED),
            is_input=False,
        )
    ...
    def __load_clutter_images(self, directory):
        ...
        struct = self.__load_images("%s/%s/structure" % (self.root, directory), self.cache_clutter_images,
                                    cv2.IMREAD_GRAYSCALE)
        struct = [(i, "structure") for i in struct]
        print("\tLoaded %i images for clutter augmentation (structure)" % len(struct))
        land = self.__load_images("%s/%s/landscape" % (self.root, directory), self.cache_clutter_images,
                                  cv2.IMREAD_GRAYSCALE)
    ...
    def __add_sea(self, image, label):
        ...
        # print(f"image/sea/label.shape {image.image.shape, sea_image.shape, label.image.shape}")
        # cv2.imshow(image)
        # cv2.waitKey()
    ...
    def __add_clutter(self, image, label):
        if np.random.random() < self.clutter_probability and image.sea_level:
            ...
            image.image[y0: image.sea_level, ...] = superimpose(image.image[y0: image.sea_level, ...], clutter,
                                                                clutter_mask)
            image.image[y0: image.sea_level, ...] = superimpose(orig_image[y0: image.sea_level, ...], clutter,
                                                                ship_mask)
        return image
